-- DESU Digital Menu Builder
-- Run this entire file in Supabase SQL Editor.
-- IMPORTANT: create the Storage bucket named "menu-images" separately in Storage,
-- or uncomment the storage INSERT if your project allows it.
...
create policy "profile self read" on public.profiles for select to authenticated using (id=auth.uid() or role='admin');
...
create policy "owner restaurant insert" on public.restaurants for insert to authenticated with check (owner_id=auth.uid());

drop policy if exists "owner restaurant update" on public.restaurants;
create policy "owner restaurant update" on public.restaurants for update to authenticated using (owner_id=auth.uid()) with check (owner_id=auth.uid());

drop policy if exists "owner restaurant delete" on public.restaurants;
create policy "owner restaurant delete" on public.restaurants for delete to authenticated using (owner_id=auth.uid());
...
create policy "public item read" on public.menu_items for select using (true);
...
-- Payments are private; server-side Chapa functions use service role.
...
-- Storage bucket. If this insert fails in your project, create a public bucket named
-- menu-images manually in Dashboard > Storage and continue.
insert into storage.buckets (id,name,public)
values ('menu-images','menu-images',true)
on conflict (id) do nothing;

drop policy if exists "public menu image read" on storage.objects;
create policy "public menu image read" on storage.objects for select using (bucket_id='menu-images');

drop policy if exists "authenticated menu image upload" on storage.objects;
create policy "authenticated menu image upload" on storage.objects for insert to authenticated
with check (bucket_id='menu-images');

drop policy if exists "authenticated menu image update" on storage.objects;
create policy "authenticated menu image update" on storage.objects for update to authenticated
using (bucket_id='menu-images') with check (bucket_id='menu-images');

drop policy if exists "authenticated menu image delete" on storage.objects;
create policy "authenticated menu image delete" on storage.objects for delete to authenticated
